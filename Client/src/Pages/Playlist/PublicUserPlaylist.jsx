...
const PublicUserPlaylist = () => {
  ...
  const getPublicPlaylistDetail = async (playlistId) => {
    try {
      const result = await http.get(`auth/music/get_public_playlist_details?playlistId=${playlistId}`)
      setPlaylistDetail(result.data.data)
    } catch (error) {
      console.log(error)
      navigate('/')
    }
  ...
  const getAllPlaylistSongs = async (playlistId) => {
    ...
    while (!stopFetching) {
      try {
        const result = await http.get(`auth/music/get_public_playlist_songs?playlistId=${playlistId}&limit=${limit}&offset=${offset}`);
  ...
  useEffect(() => {
    if(playlistId){
        getPublicPlaylistDetail(playlistId)
...
export default PublicUserPlaylist