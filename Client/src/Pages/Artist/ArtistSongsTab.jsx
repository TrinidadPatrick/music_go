...
const ArtistSongsTab = () => {
    ...
    const showLoading = artistData?.songs?.isFetching === false ? false : artistData?.songs?.isFetching === true && true
    ...
    const getAllSongs = async () => {
        try {
            ...
            const result = await http.get(`music/playlist?playlistId=${artistData?.songs?.browseId}`)
    ...
    useEffect(()=>{
       if(artistData?.songs?.all?.length === artistData?.songs?.all?.length){
        getAllSongs()
       }
    }, [])
      ...
      return (
        <div className="flex flex-col flex-1 space-y-2">
            {artistData?.songs?.all?.map((song, index) => {
