...
const MainSongs = () => {
    ...
    return results.songs?.partial?.length > 0 && (
        <div className="space-y-2 flex flex-col flex-1">
            ...
            return (
                <div onClick={(e) => {e.stopPropagation(); handleSelectSong(song)}} key={index} className={`${isCurrentSong && 'bg-secondary'} relative flex items-center space-x-4 p-2 rounded-lg hover:bg-secondary/20 group cursor-pointer`}>
                    {
                        selectedItem === song &&
                    ...
                    {/* Number */}
                    <div className="w-10 text-center justify-center hidden md:flex">
                          {
                            !isCurrentSong &&
