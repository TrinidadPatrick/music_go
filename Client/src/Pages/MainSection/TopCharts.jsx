        ...
        <ul className="text-sm text-white p-2 space-y-2">
          {
            ...
            <li
              onClick={(e) => { e.stopPropagation(); handleSaveToLibrary(selectedDropdown); }}
              className="hover:bg-gray-700 p-2 rounded"
            >
              {isSaved(selectedDropdown?.videoId) ? 'Remove from Library' : 'Add to Library'}
            </li>
          }
          <li
            onClick={(e) => { e.stopPropagation(); selectPlaylist(selectedDropdown); }}
            className="hover:bg-gray-700 p-2 rounded"
          >
            Add to Playlist
          </li>
          ...
          <header className='relative p-5 border-b border-b-slate-700'>
            ...
            <div className='flex items-center gap-2 text-sm'>
              ...
              <span className='text-gray-400 line-clamp-1'>
                {selectedDropdown?.artists ? selectedDropdown?.artists.map((artist) => artist.name).join(', ') : ''}
              </span>
            </div>
            <button
            ...
            charts?.map((chart, index) => {
              ...
              return (
                <div
                  ...
                  key={index}
                  className='group w-full cursor-pointer relative bg-gradient-to-br from-black/20 to-black/10 backdrop-blur-lg 
                  rounded-2xl p-3 transition-all duration-500 overflow-show hover:shadow-2xl border border-white/10 
                  flex gap-3 justify-between items-center flex-shrink-0 z-0'
                >
                  {/* Hover overlay */}
                  ...
                  {/* More button */}
                  {
                    isAuthenticated &&
                    <button
                      onClick={(e) => { e.stopPropagation(); handleMoreOption(chart); }}
                      className='relative z-40 cursor-pointer shrink-0 p-1 hover:bg-gray-200/10 rounded-full h-fit'
                    >
